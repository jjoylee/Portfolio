...
2. 나라마다 다른 주소 체계를 가지고 있다. 주문할 때 주소를 받아 DB에 저장해야 한다.  
...
2. 주소를 json 문자열로 변환 후 DB에 저장해 어떤 나라의 주소라도 하나의 컬럼에 저장할 수 있다.    
→ 예를 들면 미국 주소는 { "주이름" : "CA", "도시명" : "LA", "세부주소" : "new york apartment" } 처럼 들어간다.  
&nbsp;&nbsp;&nbsp;&nbsp;키 값들은 db에서 설정할 수 있다.
...
3. 주소 포맷과 json 주소 객체 key 값들을 db에 저장.   
→ 화면에 주소를 표시할 때 db에 저장한 주소 데이터(json address)를 주소 포맷에 맞춰 변환한다.  
